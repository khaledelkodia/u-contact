 ...
 *
 * **والثالث: أُلغي.** المجدولة قائمةُ عملٍ ينتظر التنفيذ، والملغيُّ ليس عملاً.
 * وكان يبقى فيها كأنّه موعدٌ قائم، **ولا يظهر في «الطلبات»** أيضاً — لأن شاشة
 * الطلبات تستثني ما لم يغادر الطابور. فيختفي من الشاشتين معاً: لا الوكيل يراه
 * ملغيّاً، ولا يجد جواباً حين يسأل العميل «أنا كنت حاجز، إيه اللي حصل؟».
 *
 * وبهذا الشرط الواحد تُحلّ المسألة كلُّها: يختفي من «المجدولة»، ويظهر في
 * «الطلبات» بلونه الأحمر، ويذهب مع يوم عمله لأن تلك الشاشة مقصورةٌ على يومها.
 */
export function schedLeftQueue(o: any): boolean {
  return schedWentLive(o) || schedIsDue(o) || o.status === 'cancelled'
}
